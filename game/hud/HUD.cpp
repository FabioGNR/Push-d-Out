...
namespace hud {
    ...
    {
        m_fpsKeySubscription = m_inputManager.subscribe([&](engine::input::maps::InputMap inputMap, auto&) {
            if (inputMap.getKeyState(engine::input::Keys::F1) == engine::input::KeyStates::PRESSED) {
                m_showFps = !m_showFps;
            }
        });
    }
